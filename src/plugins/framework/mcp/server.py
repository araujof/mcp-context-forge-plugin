...
config_file = os.environ.get("MCPCF_PLUGIN_CONFIG", "resources/plugins/config.yaml")
...
async def prompt_pre_fetch(plugin_name: str, payload: dict, context: dict) -> dict:
    ...
    """
    logger.error("ENTERING PROMPT_PRE_FETCH")
    ...
    logger.error(f"Plugin name: {plugin_name}, {plugin}, {global_plugin_manager.plugin_count}")
    try:
        if plugin:
